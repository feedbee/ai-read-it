...
const LanguageDetectionPlugin = require('../helpers/language-detection.js');
const Provider = require('./provider.js');

const GOOGLE_TTS_MAX_INPUT = 2500; // assuming 2 chars per byte TODO: logic to be changed
 ...
 */
const CreateGoogleProvider = (apiKey, options = {}) => {
...
class GoogleProvider extends Provider {
     ...
     * @param {bool} [options.detect_language] - Optional. Try to detect text language with Google Translate API if the language is not provided explicitely in textToSpeech options
     */
    constructor(options) {
        super(options);
        ...
        if (options.detect_language) {
            this.#languageDetectionPlugin = new LanguageDetectionPlugin(options);
        }
    }
     ...
     * @param {string} options.model - The model to use for the conversion (default: 'Standard').
     * @param {string} options.language – The language-region code (BCP-47) for non-multilanguage models
    ...
    async textToSpeech(text, options = {}) {
        ...
        let {
            ...
            language = !this.#languageDetectionPlugin ? 'en-US' : // BCP-47 language tag, e.g. "en-US"
                await this.#languageDetectionPlugin.detectLanguageAndRegion(text.substring(0, 500))
        } = options;

        if (response_format.toLocaleLowerCase() === 'opus') {
