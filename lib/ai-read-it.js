...
function init(apiKey, providerName) {
    if (!providerName || providerName.toLocaleLowerCase() === 'openai') {
        openaiProviderInstance = CreateOpenAIProvider(apiKey);
    } else if (providerName.toLocaleLowerCase() === 'google') {
        openaiProviderInstance = CreateGoogleProvider(apiKey);
    } else {
...
async function mediumTextToSpeech(text, options = {}) {
    ...
    // Split into chunks
    const chunks = splitTextIntoChunks(text, openaiProviderInstance.maxInputLength);
...
async function* largeTextToSpeech(text, options = {}) {
    ...
    // Split into chunks
    let chunkSize = 'chunkSize' in options ? options.chunkSize : openaiProviderInstance.maxInputLength;
...
function maxInputLength() {
    return openaiProviderInstance.maxInputLength;
}
...
class AiReadIt {
    ...
    constructor(provider, options) {
        if (typeof provider === 'string') {
            if (options && options.apiKey) {
                throw new Error('options.apiKey must be provided when provider is a string');
            }
            this.#provider = createProvider(options.apiKey);
        } else if (provider instanceof Provider) {
            this.#provider = provider;
        } else {
            throw new Error('provider is neither string nor instance of Provider class');
        }
    }
    ...
    async smallTextToSpeech(text, options = {}) {
        ...
        return this.#provider.textToSpeech(text);
    }
...
module.exports = { init, smallTextToSpeech, mediumTextToSpeech, largeTextToSpeech, maxInputLength, AiReadIt, createProvider };
